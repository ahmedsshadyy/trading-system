...
def main() -> None:
    ...
    df = add_fvg(df)
    df = add_ob(df)
    df = add_choch_context(df, include_forward_diagnostics=True)
    ...
    title = (
        "CHoCH Context Validation — XAU_USD H4 "
    ...
    result = validate_choch_context(
        plot_df,
        outpath=OUT_DIR / "choch_context_validation_XAU_USD_H4.html",
        title=title,
    )

    print("\n=== CHOCH CONTEXT SUMMARY ===")
